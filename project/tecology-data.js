// Tecology — almacén de datos compartido del prototipo.
// Simula el rol de Supabase: el catálogo y el panel admin leen y escriben aquí.
// Persiste en localStorage del navegador. En producción, estas funciones se
// reemplazan por llamadas al SDK de Supabase (tablas: products, combos, leads).
window.TecologyData = (function () {
  var KEY = 'tecology_catalog_v2';
  var LEADS_KEY = 'tecology_leads_v1';
  ...
  function load() {
    try {
      ...
      if (s) {
        var d = JSON.parse(s);
        if (!d.useCases || !d.useCases.length) d.useCases = clone(defaults.useCases);
        return d;
      }
    } catch (e) {}
    return clone(defaults);
  }
  function save(d) { try { localStorage.setItem(KEY, JSON.stringify(d)); } catch (e) { console.error('Tecology: no se pudo guardar (posible límite de almacenamiento):', e); alert('No se pudo guardar: la imagen es muy pesada o se llenó el almacenamiento local. Intenta con una imagen más pequeña.'); } }
  function reset() { try { localStorage.removeItem(KEY); } catch (e) {} }

  function getLeads() {
    try { var s = localStorage.getItem(LEADS_KEY); if (s) return JSON.parse(s); } catch (e) {}
    return [];
  }
  function saveLeads(l) { try { localStorage.setItem(LEADS_KEY, JSON.stringify(l)); } catch (e) {} }
  function addLead(lead) {
    var leads = getLeads();
    var id = 'ld-' + Date.now();
    var rec = { id: id, createdAt: new Date().toISOString() };
    for (var k in lead) rec[k] = lead[k];
    leads.push(rec);
    saveLeads(leads);
    return id;
  }
  function updateLead(id, patch) {
    ...
    for (var i = 0; i < leads.length; i++) {
      if (leads[i].id === id) { for (var k in patch) leads[i][k] = patch[k]; break; }
    }
    saveLeads(leads);
  }
  function clearLeads() { try { localStorage.removeItem(LEADS_KEY); } catch (e) {} }

  return { load: load, save: save, reset: reset, defaults: defaults,
           getLeads: getLeads, addLead: addLead, updateLead: updateLead, clearLeads: clearLeads };
})();
